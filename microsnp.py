...
def parse_args():
    ...
    filterdpgq =  subparser.add_parser('filter_dp_gq',parents=[parent_parser,dp_parser,gq_parser],help='Filters variants that have DP > min_dp and GQ > min_gq.')
    filterdpgq.required = True
    filterdpgq.set_defaults(func=filter_dp_gq)

    filterdp =  subparser.add_parser('filter_dp',parents=[parent_parser,dp_parser],help='Filters variants across all values and samples that have DP > DP specified')
