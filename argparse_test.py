...
def parse_args():
    ...
    mpileup_parser.add_argument("path_to_files", type=str, help='Path to files')
    mpileup_parser.add_argument("file_extension", metavar='file_ext', type=str, help='File extension e.g. \'.bam\' \'.sorted.bam\'')
    ...
    bcftools_single = subparser.add_parser('mpileup_single',parents=[mpileup_parser], help='Run an mpileup on a single sample vs. a reference genome.',
    ...
    bcftools_multi = subparser.add_parser('mpileup_multi',parents=[mpileup_parser], help='run an mpileup on multiple samples vs. a reference genome.',
    ...
    # bcftools_parser_multi.required = Trues
    # bcftools_parser_multi.set_defaults(func=mpileup_multi)




    # Add parser which takes the same arguments as bcftools
    # bcftools_parser_multi = subparser.add_parser('mpileup_multi',parent=[bcftools_parser], help='run an mpileup on multiple samples vs. a reference genome.')
    ...
    # bcftools_index = subparser.add_parser('index_vcf',help='Indexes all files with specified file_extension in path.')
    # bcftools_index.required = True
    # bcftools_index.set_defaults(func=index_vcf)

    # htsfile_parser = subparser.add_parser('htsfile',parents=[bcftools_compress],help='Output compression status all files in path_to_files with specified file_extension')
...
def mpileup_multi(args): #ath_to_files, file_extension, path_to_reference_file, depth
    ...
    for f in files:
        ...
        try:
            #command=f'bcftools mpileup -Ou --max-depth 8000 --min-MQ 30 --min-BQ 30 -f {args.path_to_ref}  {f} | bcftools call --ploidy 1 -Ou -mv |  bcftools filter -s LowQual -e \'%QUAL<20\'  > {args.path_to_output}/{subject_clean}_{sample_id_clean}.flt.vcf'
            command=f'bcftools mpileup -Ou --max-depth 8000 --min-MQ 30 --min-BQ 30 -f {args.path_to_ref} {subject_path}/*{args.file_extension} | bcftools call --ploidy 1 -mv -Ou |  bcftools filter -s LowQual -e \'%QUAL<20\'  > {args.path_to_output}/{subject_clean}.raw.bcf'
